...
//  pathplan::ReplannerManagerToGoalPtr rm = std::make_shared<pathplan::ReplannerManagerToGoal>(current_path,solver,nh);
//  pathplan::ReplannerBasePtr rep = rm->getReplanner();

//  ROS_INFO_STREAM("rep "<<rep);
...
//  rm->startReplannedPathFromNewCurrentConf(new_current_configuration);
